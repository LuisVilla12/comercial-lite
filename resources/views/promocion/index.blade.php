@section('title', content: 'Promociones')
...
<x-app-layout>
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 my-4">
        {{-- Buscador --}}
        <form method="GET" action="{{ route('promociones.index') }}" class="w-full md:w-1/3">
            <div class="relative">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Buscar promocion..."
    ...
    <div class="shadow-md overflow-x-auto rounded-lg ">
                            ...
                            <tr class="border-t">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-700">
                                    <div class="flex justify-center items-center gap-4">
                                        {{-- Ver --}}
                                        <a href="{{ route('promociones.show', ['promocion' => $promocion]) }}"
                                        ...
                                        {{-- Editar --}}
                                        <a href="{{ route('promociones.edit', ['promocion' => $promocion]) }}"
                                        ...
                                        {{-- Eliminar --}}
                                        <form action="{{ route('promociones.destroy', ['promocion' => $promocion]) }}"
                ...
                @foreach ($promociones as $promocion)
                    <div class="border rounded-lg shadow bg-white p-4">
                        ...
                        <div class="flex flex-wrap items-center justify-end mt-4 gap-4">
                            ...
                            {{-- Eliminar --}}
                            <form action="{{ route('promociones.destroy', ['promocion' => $promocion]) }}" method="POST"
                                class="inline">
                                @csrf
                                @method('DELETE')
        ...
        @else
            <div class="bg-white py-4 mt-3">
                <p class="text-sm text-gray-600 ml-6 text-center"> No hay promociones registrados</p>
            </div>
        @endif
        @if ($promociones->count() > 0)
            <div class="bg-white py-4 my-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                ...
                <p class="text-sm text-gray-600 ml-6">
                    Mostrando
                    <span class="font-medium">{{ $promociones->firstItem() }}</span>
                    a
                    <span class="font-medium">{{ $promociones->lastItem() }}</span>
                    de
                    <span class="font-medium">{{ $promociones->total() }}</span>
                    registros
                </p>

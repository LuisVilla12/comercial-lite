@section('title', content: 'Medios de pago')

<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200">
            Catálogo de Metodos de pago
        </h2>
    </x-slot>
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 my-4">

        {{-- Buscador --}}
        <form method="GET" action="{{ route('metodos.index') }}" class="w-full md:w-1/3">
            <div class="relative">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Buscar medio de pago..."
                            ...
                            <tr class="border-t">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-700">
                                    <div class="flex justify-center items-center gap-4">
                                        {{-- Ver --}}
                                        <a href="{{ route('metodos.show',$medio) }}"
                                        ...
                                        {{-- Editar --}}
                                        <a href="{{ route('metodos.edit',$medio) }}"
                                        ...
                                        {{-- Eliminar --}}
                                        <form action="{{ route('metodos.destroy',$medio) }}"
                ...
                @foreach ($medios as $medio)
                    <div class="border rounded-lg shadow bg-white p-4">
                        ...
                        <div class="flex flex-wrap items-center justify-end mt-4 gap-4">
                            ...
                            {{-- Eliminar --}}
                            <form action="{{ route('metodos.destroy',$medio) }}" method="POST"
                                class="inline">
                                @csrf
                                @method('DELETE')

...
<x-app-layout>
      ...
      <div class="flex items-center mt-4 py-2 gap-3 mb-4 bg-white dark:bg-slate-800 w-full rounded-md">
        ...
        <div class="">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200">
                Modificar un REP #{{ $documento->id }}
            </h2>
            <p class="dark:text-white mt-2 font-semibold"> Fecha:
                {{ now()->format('d/m/Y') }}</span></p>
        </div>
    </div>
    <form method="POST" action="{{ route('pagos.update',$documento) }}" x-data="documentoEdit(@js($documento->toArray()))" x-init="init();">
        @method('PUT')
        @csrf
    ...
    "
            ...
            <div x-show="tab === 'info'" x-cloak class="space-y-4">
                <div class="md:grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 md:gap-4 lg:gap-4  bg-white dark:bg-slate-800  rounded-md p-2 mt-2">
                    <div class="col-span-full">
                        <label
                            class="block text-lg font-medium mb-2 dark:text-white">
            ...
            return {
                ...
                init() {
                     // COLOCA LOS PRODUCTOS
                        this.facturas = documento.detalles.map(d => ({
                            id: d.id,
                            serie: d.documento.serie,
                            folio: d.documento.folio,
                            fecha: d.documento.fecha,
                            total: d.documento.total,
                            saldo_pendiente: d.documento.saldo_pendiente,
                            saldo_captura: Number(Number(d.documento.saldo_pendiente).toFixed(2)),
                            monto: d.monto,
                        }))
                },

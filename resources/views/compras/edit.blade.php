        ...
        <x-slot name="header">
            <div class="md:flex md:justify-between">
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 text-center">
                    Actualizar una compra                </h2>
                <label class="block text-lg font-medium mb-2 dark:text-white text-center">Fecha: {{ now()->format('d/m/Y') }}
                </label>
            </div>
        </x-slot>

        <form method="POST" action="{{ route('compras.update', $compra) }}" x-data="compraEdit(@js($compra->toArray()))"
            x-init="init();
                    ...
                    <div class=" mx-auto py-6">
                        <div class="md:flex gap-5 md:justify-between">
                            <div class="w-full">
                                ...
                                <input type="text" x-model="proveedorQuery" autofocus readonly
...
<label class="block text-lg font-medium mb-2 dark:text-white">Seleccionar almacen: *</label>
                            <select name="almacen_id" id="almacen_id" x-model="almacen_origen_id"
                                ...
                                @foreach ($almacenes as $almacen)
                                    <option  @selected($compra->almacen_id == $almacen->id) value="{{ $almacen->id }}">{{ $almacen->nombre }}</option>
                                @endforeach
                            </select>
                            </div>
                        </div>
        ...
        <script>
            ...
            function compraEdit(compra) {
                return {
                    ...
                    almacen_origen_id: compra.almacen_id,
                    proveedores: [],
                    proveedorSeleccionado: -1,

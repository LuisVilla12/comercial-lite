...
<x-app-layout>
 ...
 class="px-4 py-2 bg-blue-600 text-white rounded  flex items-center md:ml-6">
...
@if($existencia->producto->maximominimo->isNotEmpty())
                                    @foreach ($existencia->producto->maximominimo as $registro)
                                        @if($registro->almacen_id ==  $existencia->almacen_id)
                                    <p class="mb-2 text-sm py-1 text-center font-semibold {{ $existencia->cantidad<=$registro->minimo?'bg-red-300':'bg-green-200'  }}">

                                        Minimo: {{ $registro->minimo }}
                                    </p>
                                    <p class="mb-2 text-sm py-1 text-center font-semibold {{ $existencia->cantidad>=$registro->maximo?'bg-red-300':'bg-green-200'  }}">
